...
class Trainer:
    ...
    def run_epoch(self, epoch, data_loader, mode):
        ...
        for iter_id, meta in enumerate(data_loader):
            ...
            if iter_id % self.cfg.log.interval == 0:
                log_msg = '{}|Epoch{}/{}|Iter{}({}/{})| lr:{:.3e}| '.format(mode, epoch, self.cfg.schedule.total_epochs,
                                                                            self._iter, iter_id, num_iters,
            ...
            if iter_id % self.cfg.schedule.save_interval == 0:
                save_model(self.rank,
                           self.model,
                           os.path.join(self.cfg.save_dir, 'epoch{:d}_iter{:d}.pth'.format(epoch, iter_id)),
                           epoch,
                           self._iter,
                           self.optimizer)
    ...
    def run(self, train_loader, val_loader, evaluator):
        ...
        for epoch_i, epoch in enumerate(range(start_epoch, self.cfg.schedule.total_epochs + 1)):
            ...
            if evaluator is None:
                ...
                best_save_path = os.path.join(self.cfg.save_dir, 'epoch_{:d}'.format(start_epoch + epoch_i))
            ...
            for iter_id, batch in enumerate(data_loader):
                ...
                for k in loss_stats:
                    if k not in step_losses:
                        ...
                        step_losses[k].push(loss_stats[k].mean().item())
                if iter_id % self.cfg.log.interval == 0:
                    log_msg = '{}|Iter({}/{})| lr:{:.2e}| '.format('warmup', cur_iter, num_iters,
                                                                   self.optimizer.param_groups[0]['lr'])
                    for l in step_losses:
                        log_msg += '{}:{:.4f}| '.format(l, step_losses[l].avg())
                    self.logger.log(log_msg)
    ...
    def resume(self, cfg):
        ...
        if 'optimizer' in checkpoint:
            ...
            self.optimizer.param_groups[0]['lr'] = self.cfg.schedule.optimizer.lr
